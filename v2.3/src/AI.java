 ...
 * Spring 2013
 * 
 * This class is the AI that powers
 * the game if the user wants to 
...
public class AI extends Observable implements Runnable
{
	...
	private int maxDepth = 9;	// Max depth for Minimax
	private Board board;
	private int player;
	...
	public Move getBestMove(Board newBoard)
	{
		...
		Move[] allMoves = getAllMoves(newBoard, pieces, null, true);
		...
		for (Move currMove: allMoves)
		{
			...
			newBoard.moveAI(currMove.from, currMove.to, currMove.currPiece, 0, maxDepth);
			
			int score = min(newBoard, Integer.MIN_VALUE, Integer.MAX_VALUE, 1, currMove.from);
	
			newBoard.unMoveAI(currMove.from, currMove.to, consumed, currMove.currPiece, 0, maxDepth);
	
			if (score == bestscore)
			{
				bestMoves.put(currMove.to, currMove.from);
			}
	...
	private int min (Board newBoard, int alpha, int beta, int depth, Location prev)
	{
		...
		//Get a list of all possible moves.
		Move[] allMoves = getAllMoves(newBoard, pieces, prev, false);
	...
	private Move[] getAllMoves(Board newBoard, Location[] pieces, Location prev, boolean isRoot)
	{
		ArrayList<Move> moves = new ArrayList<Move>();
		
		for (int currPiece = 0; currPiece < pieces.length; ++currPiece)
		{
			Location from = pieces[currPiece];
			int row = from.getRow();
			int col = from.getCol();
			...
			for (int currRow = -1; currRow < 2; ++currRow)
			{
				for (int currCol = -1; currCol < 2; ++currCol)
				{
					...
					if (!(currRow == -1 && u) && !(currRow == 1 && d) && !(currCol == -1 && l) && !(currCol == 1 && r))
					{
						...
						if (prev != null && to.equals(prev))
							continue;

						if (!newBoard.doesPieceExist(to))
						{
							...
							if (isRoot || (curr.value != 0 || curr.isCornerMove()))
								moves.add(curr);
						}
					}
				}
	...
	public void run() {
		while (!board.endOfGame() || continueGame)
		{
			try {
				...
				potentialMove = getBestMove(newBoard);
				
				//Notify Game that AI wants to make a move
				setChanged();  
				notifyObservers();
